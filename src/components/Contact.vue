<template>
    <div class="mt-10 py-3 bg-gray-900" id="contact">
        <h3 class="mb-4 text-white hover:text-gray-700 duration-300 text-3xl w-20 mx-auto">Contact</h3>
        <div class="px-2 py-3 block bg-gray-500 border-0 rounded mx-auto" style="max-width: 800px;">
            <div class="grid grid-cols-2 grid-rows-1">
                <div class="col-span-1 mx-auto pl-3 flex flex-col gap-2 text-white underline">
                    <a href="mailto:nyinyi@gmail.com" class="hover:text-gray-400 hover:translate-x-1 duration-200">nyinyi@gmail.com</a>
                    <a href="tel:9454983001" class="hover:text-gray-400 hover:translate-x-1 duration-200">+959454983001</a>
                    <a href="viber:9454983001" class="hover:text-gray-400 hover:translate-x-1 duration-200">+959454983001</a>
                </div>
                <div class="col-span-1 mx-auto pl-3 flex flex-col gap-4 text-white">
                    <div class="flex gap-5">
                        <font-awesome-icon icon="fa-brands fa-facebook" class="text-3xl hover:text-blue-400 hover:scale-105 duration-300 cursor-pointer"></font-awesome-icon>
                        <font-awesome-icon icon="fa-brands fa-github" class="text-3xl hover:text-blue-300 hover:scale-105 duration-300 cursor-pointer"></font-awesome-icon>
                    </div>
                    <div class="flex gap-5">
                        <font-awesome-icon icon="fa-brands fa-linkedin" class=" text-3xl hover:text-blue-400 hover:scale-105 duration-300 cursor-pointer"></font-awesome-icon>
                        <font-awesome-icon icon="fa-brands fa-twitter" class="text-3xl hover:text-blue-400 hover:scale-105 duration-300 cursor-pointer"></font-awesome-icon>
                    </div>
                </div>
            </div>
        </div>
    </div>
</template>

<script setup>

</script>

<style scoped>

</style>